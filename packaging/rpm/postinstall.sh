...
if command -v systemctl >/dev/null 2>&1; then
    ...
    echo "Run as the service user so the SQLite DB is writable:"
    echo "  sudo -u pertisk-chart pertisk-chart-create-admin \\"
    echo "    -username admin \\"
    ...
    echo "    -db-type sqlite"
    echo ""
    echo "If you already created the DB as root, fix ownership:"
    echo "  sudo chown -R pertisk-chart:pertisk-chart /var/lib/pertisk-chart"
    echo ""
